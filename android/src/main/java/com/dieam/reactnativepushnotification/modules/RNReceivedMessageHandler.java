    ...
    public void handleReceivedMessage(RemoteMessage message) {
        ...
        if (notificationData.containsKey("sendbird")) {
            if (notificationData.containsKey("message") && notificationData.get("message").split(":").length >= 2) {
                ...
                String mes = notificationData.get("message");
                if (mes == null) {
                    mes = "Image/Document Shared";
                }
                if (!bundle.containsKey("message")) {
                    bundle.putString("message", mes);
                }
                if (!bundle.containsKey("title")) {
        ...
        if(notificationData.containsKey("ongoingDocon")) {
            bundle.putString("message", notificationData.optString("body", null));
            bundle.putString("title", notificationData.optString("title", null));
        }
